...
class Autorepl_Public
{
  public function add_head_code()
  {
    ...
    wp_add_inline_script('autorepl', $this->parse_wp_user());
  ...
  {
    $url = str_starts_with(get_option('url'), 'https://autorepl.com')
      ? 'https://bot.autorepl.com' . '/' . explode('/', get_option('url'))[3]
      : get_option('url');
    if (!$url) {
      return;
    }
    if (
      ...
      $arr_js = $arr_js . ']';
      wp_add_inline_script('autorepl', $arr_js);
    } else {
      wp_add_inline_script('autorepl', 'const typebot_include_paths = null');
    }
    $params =
      '{
					url: "' .
  ...
  public function add_autorepl_container($attributes = [])
  {
    $width = '100%';
    $height = '500px';
    ...
    $container_id = 'autorepl-container-' . $this->generateRandomString(4);

    $bot_initializer =
      'var autorepl = AutoRepl.initContainer("' .
      $container_id .
      '",{
    hiddenVariables: typebotWpUser,
          url: "' .
